...
[SerializeField]
class LightingNode : ReflectNode<LightingFilter>
{
     ...
     protected override LightingFilter Create(ReflectBootstrapper hook, ISyncModelProvider provider, IExposedPropertyTable resolver)
     {
         Debug.Log("created lighting filter");
         var filter = new LightingFilter();
...
class LightingFilter : IReflectNodeProcessor
{
    class FilterData
    {
    ...
    private LightGrouper lightGrouper;

    public void SetupGrouper(LightGrouper grouper)
    {
        lightGrouper = grouper;
        lightGrouper.SearchParameter = "Switch ID";
    }
    ...
    public void OnStreamInstanceEvent(SyncedData<StreamInstance> stream, StreamEvent streamEvent)
    {
        ...
        if (metadata != null && metadata.Parameters.TryGetValue("Switch ID", out var category))
        {
    ...
    void OnGameObjectAdded(GameObject gameObject)
    {
        ...
        if (metadata.parameters.dictionary.TryGetValue("Switch ID", out var category))
        {
            ...
            lightGrouper.FindDeviceIDs(metadata);
