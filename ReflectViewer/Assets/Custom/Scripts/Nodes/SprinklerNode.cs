...
class SprinklerNode : ReflectNode<SprinklerFilter>
{
    ...
    protected override SprinklerFilter Create(ReflectBootstrapper hook, ISyncModelProvider provider, IExposedPropertyTable resolver)
    {
        Debug.Log("created category filter");
        var filter = new SprinklerFilter();
...
class SprinklerFilter : IReflectNodeProcessor
{
    class FilterData
    {
    ...
    //custom
    private SprinklerGrouper sprinklerGrouper;

    public void SetupGrouper(SprinklerGrouper grouper)
    {
        Debug.Log("setting up Sprinkler Filter");
        sprinklerGrouper = grouper;
        sprinklerGrouper.SearchParameter = "Id";
    }
    //
    ...
    void OnGameObjectAdded(GameObject gameObject)
    {
        ...
        if (metadata.parameters.dictionary.TryGetValue("Category", out var category))
        {
            ...
            //custom
            if (category.value.Equals("Sprinklers"))
                sprinklerGrouper.FindDeviceIDs(metadata);
