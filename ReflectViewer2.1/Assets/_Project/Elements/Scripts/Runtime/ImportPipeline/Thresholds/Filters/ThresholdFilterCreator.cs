...
{
    public class ThresholdFilterCreator : CustomFilter
    {
        #region VARIABLES

        private ThresholdFilter _nodeProcessorFilter;
        private ThresholdManager _manager;

        #endregion

        #region CUSTOM METHODS

        private IEnumerator SetupRoutine(ThresholdNode node, ThresholdManager m)
        {
        ...
        {
            if (PipelineAsset.TryGetNode(out ThresholdNode reflectNode))
            {
                // YOUR CUSTOM CODE GOES HERE
                // YOUR CODE - START
            ...
            ThresholdNode node = PipelineAsset.CreateNode<ThresholdNode>();
        ...
        {
            _manager = node as ThresholdManager;
        }
