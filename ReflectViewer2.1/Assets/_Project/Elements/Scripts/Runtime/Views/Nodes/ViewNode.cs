...
namespace Zutari.Elements.Nodes
{
    ...
    [Serializable]
    public class ViewNode : ReflectNode<ViewFilter>
    {
        ...
        protected override ViewFilter Create(ReflectBootstrapper hook, ISyncModelProvider provider,
                                                      IExposedPropertyTable resolver)
        {
            Debug.Log("Created new ViewFilter.");
            ViewFilter ViewFilter = new ViewFilter();

            InstanceInput.streamEvent = ViewFilter.OnStreamInstanceEvent;
            GameObjectInput.streamEvent = ViewFilter.OnGameObjectEvent;
    ...
    public class ViewFilter : IReflectNodeProcessor
    {
        ...
        public void Setup(ViewManager node)
        {
            viewManager = node;
        }
        ...
        {
            Debug.Log("view node set up");
            viewManager = node as ViewManager;
        }
        ...
        private void OnGameObjectAdded(GameObject gameObject)
        {
            ...
            if (metadata.parameters.dictionary.TryGetValue("Category", out Metadata.Parameter category))
            {
                ...
                if (category.value.Equals("Views"))
                {
        ...
        private void OnGameObjectRemoved(GameObject gameObject)
        {
            ...
            if (metadata.parameters.dictionary.TryGetValue("View", out Metadata.Parameter category))
            {
                if (_instances.TryGetValue(category.value, out FilterData filter))
                {
