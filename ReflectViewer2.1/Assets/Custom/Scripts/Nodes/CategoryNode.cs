...
[SerializeField]
class CategoryNode : ReflectNode<CategoryFilter>
{
    ...
    protected override CategoryFilter Create(ReflectBootstrapper hook, ISyncModelProvider provider, IExposedPropertyTable resolver)
    {
...
class CategoryFilter : IReflectNodeProcessor
{
    class FilterData
    {
    ...
    //custom
    private SortCategories categorySorter;

    public void Setup(Transform parent)
    {
        Debug.Log("setting up Category Filter");
        categorySorter = new SortCategories();
        categorySorter.SearchParameter = "Category";
        categorySorter.root = parent;
    }
    ...
    void OnGameObjectAdded(GameObject gameObject)
    {
        ...
        if (metadata.parameters.dictionary.TryGetValue("Category", out var category))
        {
            ...
            //custom
            categorySorter.SortObject(metadata);
